# Refutation brief — bead contest-wrp.7 — PR #2 — cycle 3
...
## Cycle-3 addendum (supersedes the cycle-2 addendum above)

Review head `1befa1d`. Cycle 2 raised one concern (the documented CONTEST_TEST_DATABASE_URL override failed by construction against a hard-coded default assertion) and one note (bracketed IPv6 [::1] allowlisted but not connectable). The concern is claimed fixed. The IPv6 note was deliberately left alone and captured as follow-up — do NOT regrade it as a blocker.

Attack the new claim: does honouring a loopback override reopen any redirect path that cycle 1 and cycle 2 closed? Confirm the query-host, socket-path, repeated-host, percent-encoded, and case-variant redirects are all still rejected, and that the pinning invariant (an ambient DATABASE_URL cannot survive) still holds.

Convergence discipline: this is cycle three. A finding may block only if it is destructive or belongs to a class no earlier cycle adjudicated. Anything in the host-validation or configuration-contract classes is follow-up work, and you must say so explicitly when grading it.
