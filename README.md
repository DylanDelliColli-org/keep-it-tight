...
`npm run seed:members` passes credentials through environment variables,
normally loaded from `.env.local`; those values are the three real account
passwords, and the gitignored `.env.local` file must never be committed.
